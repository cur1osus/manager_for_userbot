...
class TTLCache:
    ...
    async def set(
        self, key: KeyValueT, time_ms: ExpiryT, value: Any, throttle_key: str = "-"
    ) -> bool:
...
class LeakyBucket:
    ...
    async def is_limit_reached(
        self, object_id: KeyValueT, bucket_decrement: int = 1
    ) -> bool:
...
class ThrottlingMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        if isinstance(throttle_time, timedelta):
            throttle_time = int(
                throttle_time.total_seconds() * 1000
        ...
        if await self.leaky_bucket.is_limit_reached(
            user.id, bucket_decrement=bucket_decrement
        ):
            if isinstance(event, CallbackQuery):
                await event.answer("🪣 Too fast!", show_alert=True)
